...
export class ExamsMockService {
  ...
  // Helpers
  listTables() {
    return this.examTables();
  }
  listFinalsByTable(tableId: number) {
    return this.finalExams().filter((e) => e.exam_table_id === tableId);
  }
  getTable(id: number) {
    return this.examTables().find((t) => t.id === id) || null;
  }
  getFinal(id: number) {
    return this.finalExams().find((f) => f.id === id) || null;
  }
  ...
  updateTable(id: number, patch: Partial<ExamTable>) {
    this.examTables.set(
      this.examTables().map((t) => (t.id === id ? { ...t, ...patch } : t)),
    );
  }
  deleteTable(id: number) {
    this.examTables.set(this.examTables().filter((t) => t.id !== id));
    this.finalExams.set(
      this.finalExams().filter((f) => f.exam_table_id !== id),
    );
  }
  ...
  deleteFinal(id: number) {
    ...
    this.finalExamStudents.set(
      this.finalExamStudents().filter((s) => s.final_exams_id !== id),
    );
  }
}
