...
async function main() {
  try {
    ...
    const tablesResp: any = await http("/finals/exam-table/list", {
      headers: authTeacher,
    });
    const tablesArr = Array.isArray(tablesResp?.data)
      ? tablesResp.data
      : Array.isArray(tablesResp)
        ? tablesResp
        : [];
    if (!tablesArr.length) throw new Error("No hay exam tables");
    const t0 =
      tablesArr.find((t: any) => t.name?.includes("Mesa Smoke")) ||
      tablesArr[0];
    console.log("[TEST] Using table:", t0);

    // 3) Listar exámenes de la mesa
    const examsResp: any = await http(`/finals/exam/list-all/${t0.id}`, {
      headers: authTeacher,
    });
    const examsArr = Array.isArray(examsResp?.data)
      ? examsResp.data
      : Array.isArray(examsResp)
        ? examsResp
        : [];
    ...
    const detail: any = await http(`/finals/exam/list/${e0.id}`, {
      headers: authTeacher,
    });
    ...
    const recordResp = await http("/finals/exam/record", {
      method: "POST",
      headers: { ...authTeacher },
    ...
    const approveResp = await http("/finals/exam/approve", {
      method: "POST",
      headers: { ...authTeacher },
