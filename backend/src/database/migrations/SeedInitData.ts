...
export class SeedInitialData1699999999999 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const mesaJulio = await queryRunner.query(
      `SELECT id FROM exam_table WHERE name='Mesa Julio 2025' LIMIT 1;`
    );
    const mesaSept = await queryRunner.query(
      `SELECT id FROM exam_table WHERE name='Mesa Septiembre 2025' LIMIT 1;`
    );
    const mesaMarzo = await queryRunner.query(
      `SELECT id FROM exam_table WHERE name='Mesa Marzo 2026' LIMIT 1;`
    );
