...
export class SeedInitialData1699999999999 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const tableRow = await queryRunner.query(
      `SELECT id FROM final_exam_table WHERE name = 'Mesa Diciembre 2025' ORDER BY id DESC LIMIT 1;`
    );
    const tableId: number = tableRow[0]?.id;

    await queryRunner.query(`
      INSERT INTO final_exams (final_exam_table_id, subject_id, exam_date, exam_time, aula)
      VALUES (${tableId}, ${subjectId}, CURRENT_DATE + INTERVAL '7 DAYS', '10:00'::time, 'Aula 12')
      RETURNING id;
    `);
    const finalRow = await queryRunner.query(
      `SELECT id FROM final_exams WHERE final_exam_table_id = ${tableId} AND subject_id = ${subjectId} ORDER BY id DESC LIMIT 1;`
    );
    const finalId: number = finalRow[0]?.id;

    await queryRunner.query(`
      INSERT INTO final_exams_students (final_exams_id, student_id, enrolled_at, score, notes)
      VALUES (${finalId}, '${studentUser}', CURRENT_DATE, NULL, 'Inscripto en mesa de diciembre');
    `);
    ...
    if (newSubjectId) {
      ...
      await queryRunner.query(`
        ...
        WHERE NOT EXISTS (
          SELECT 1 FROM final_exam_table WHERE name = 'Mesa Febrero 2026'
        );
      `);

      const newTableRow = await queryRunner.query(
        `SELECT id FROM final_exam_table WHERE name = 'Mesa Febrero 2026' ORDER BY id DESC LIMIT 1;`
      );
      ...
      if (newTableId) {
        ...
        await queryRunner.query(`
          INSERT INTO final_exams (final_exam_table_id, subject_id, exam_date, exam_time, aula)
          SELECT ${newTableId}, ${newSubjectId}, CURRENT_DATE + INTERVAL '62 DAYS', '10:00'::time, 'Aula 5'
          WHERE NOT EXISTS (
            SELECT 1 FROM final_exams 
            WHERE final_exam_table_id = ${newTableId} AND subject_id = ${newSubjectId}
          );
        `);
        ...
        const newFinalRow = await queryRunner.query(`
          SELECT id FROM final_exams WHERE final_exam_table_id = ${newTableId} AND subject_id = ${newSubjectId} ORDER BY id DESC LIMIT 1;
        `);
        const newFinalId: number = newFinalRow[0]?.id;
        if (newFinalId) {
          await queryRunner.query(`
            INSERT INTO final_exams_students (final_exams_id, student_id, enrolled_at, score, notes)
            SELECT ${newFinalId}, '${studentUser}', NULL, NULL, 'Disponible para inscripción'
            WHERE NOT EXISTS (
    ...
    // await queryRunner.query(
    //   `INSERT INTO teachers (user_id) VALUES ('${teacher2User}') ON CONFLICT DO NOTHING;`
    // );
    // await queryRunner.query(
    //   `INSERT INTO teachers (user_id) VALUES ('${teacher3User}') ON CONFLICT DO NOTHING;`
    // );
    // await queryRunner.query(
    //   `INSERT INTO preceptors (user_id) VALUES ('${preceptor2User}') ON CONFLICT DO NOTHING;`
    // );
    // await queryRunner.query(
    //   `INSERT INTO secretaries (user_id, is_directive) VALUES ('${secretary2User}', false) ON CONFLICT DO NOTHING;`
    // );
    // for (const stu of [
    //   student2User,
    //   student3User,
    ...
    // ]) {
    //   await queryRunner.query(
    //     `INSERT INTO students (user_id) VALUES ('${stu}') ON CONFLICT DO NOTHING;`
    //   );
    // }
    ...
    //   const addrRow = await queryRunner.query(
    //     `SELECT id FROM address_data WHERE street='${street}' AND number='${number}' ORDER BY id DESC LIMIT 1;`
    //   );
    ...
    //     SELECT '${p.id}', ${addrId ? addrId : "NULL"}, '${p.sex}', '${
    //     p.birthDate
    //   }', 'Ciudad', 'Argentina'
    ...
    //     SELECT '${p.id}', 'DNI', '${p.doc}', '+54 11 5555${(1000 + idx)
    //     .toString()
    //     .slice(-4)}', 'Contacto ${idx + 1}', '+54 11 4444${(1000 + idx)
    //     .toString()
    //     .slice(-4)}'
    ...
    //     year: "2026",
    //     correlative: "Inglés Técnico I",
    //   },
    //   // 3er Año (2027)
    //   {
    //     name: "Programación III",
    //     year: "2027",
    //     correlative: "Programación II",
    //   },
    //   {
    //     name: "Programación Web II",
    //     year: "2027",
    //     correlative: "Programación Web I",
    //   },
    //   {
    //     name: "Ingeniería de Software II",
    //     year: "2027",
    //     correlative: "Ingeniería de Software I",
    //   },
    ...
    //   await queryRunner.query(`
    //     INSERT INTO subjects (subject_name, teacher, preceptor, course_num, course_letter, course_year, correlative)
    //     SELECT '${c.name}', '${teacher}', '${preceptor}', ${courseNum}, 'A', '${
    //     c.year
    //   }',
    //       ${
    //         c.correlative
    //           ? `(SELECT id FROM subjects WHERE subject_name='${c.correlative}')`
    //           : "NULL"
    //       }
    //     WHERE NOT EXISTS (SELECT 1 FROM subjects WHERE subject_name='${
    //       c.name
    //     }');
    //   `);
    //   const row = await queryRunner.query(
    //     `SELECT id FROM subjects WHERE subject_name='${c.name}' ORDER BY id DESC LIMIT 1;`
    //   );
    ...
    //   `);
    //   const exam1 = await queryRunner.query(
    //     `SELECT id FROM exams WHERE subject_id=${id} AND title='Parcial 1' LIMIT 1;`
    //   );
    //   const exam2 = await queryRunner.query(
    //     `SELECT id FROM exams WHERE subject_id=${id} AND title='Parcial 2' LIMIT 1;`
    //   );
    //   const e1 = exam1[0]?.id;
    //   const e2 = exam2[0]?.id;
    //   if (e1) {
    ...
    //   WHERE NOT EXISTS (SELECT 1 FROM final_exam_table WHERE name='Mesa Julio 2025');
    // `);
    // await queryRunner.query(`
    //   INSERT INTO final_exam_table (name, start_date, end_date, created_by)
    //   SELECT 'Mesa Septiembre 2025', CURRENT_DATE - INTERVAL '2 DAYS', CURRENT_DATE + INTERVAL '5 DAYS', '${secretary2User}'
    //   WHERE NOT EXISTS (SELECT 1 FROM final_exam_table WHERE name='Mesa Septiembre 2025');
    // `);
    // await queryRunner.query(`
    //   INSERT INTO final_exam_table (name, start_date, end_date, created_by)
    //   SELECT 'Mesa Marzo 2026', CURRENT_DATE + INTERVAL '30 DAYS', CURRENT_DATE + INTERVAL '35 DAYS', '${secretaryUser}'
    //   WHERE NOT EXISTS (SELECT 1 FROM final_exam_table WHERE name='Mesa Marzo 2026');
    // `);
    // const mesaJulio = await queryRunner.query(
    //   `SELECT id FROM final_exam_table WHERE name='Mesa Julio 2025' LIMIT 1;`
    // );
    // const mesaSept = await queryRunner.query(
    //   `SELECT id FROM final_exam_table WHERE name='Mesa Septiembre 2025' LIMIT 1;`
    // );
    // const mesaMarzo = await queryRunner.query(
    //   `SELECT id FROM final_exam_table WHERE name='Mesa Marzo 2026' LIMIT 1;`
    // );
    // const mj = mesaJulio[0]?.id;
    // const ms = mesaSept[0]?.id;
    // const mm = mesaMarzo[0]?.id;

    // // Crear finales para algunas materias en cada mesa
    // const finalsMatrix: Array<{
    ...
    //   {
    //     tableId: ms,
    //     subjectName: "Programación Web I",
    //     offset: 3,
    //     aula: "Lab Web",
    //   },
    //   {
    //     tableId: mm,
    //     subjectName: "Proyecto Final",
    //     offset: 31,
    //     aula: "Sala Proyecto",
    //   },
    // ];
    ...
    //   const sidRow = await queryRunner.query(
    //     `SELECT id FROM subjects WHERE subject_name='${f.subjectName}' LIMIT 1;`
    //   );
    //   const sid = sidRow[0]?.id;
    //   if (!sid) continue;
    //   await queryRunner.query(`
    //     INSERT INTO final_exams (final_exam_table_id, subject_id, exam_date, aula)
    //     SELECT ${f.tableId}, ${sid}, CURRENT_DATE + INTERVAL '${f.offset} DAYS', '${f.aula}'
    //     WHERE NOT EXISTS (SELECT 1 FROM final_exams WHERE final_exam_table_id=${f.tableId} AND subject_id=${sid});
    //   `);
    // }

    // // Relacion final_exams_students: inscribir parcialmente
    // const finals = await queryRunner.query(
    //   `SELECT id, subject_id, exam_date FROM final_exams;`
    // );
    ...
    //     const scoreNumeric =
    //       isPast && enrolled ? 6 + (hashSeed % 40) / 10 : null; // 6.0 .. 9.9
    //     const enrolledAtExpr = enrolled
    //       ? "CURRENT_DATE - INTERVAL '1 DAY'"
    //       : "NULL";
    //     const scoreExpr =
    //       scoreNumeric !== null ? `'${scoreNumeric.toFixed(2)}'` : "NULL";
    //     const notesExpr =
    //       scoreNumeric !== null ? `'Calificado'` : `'Pendiente'`;
    //     await queryRunner.query(`
    //       INSERT INTO final_exams_students (final_exams_id, student_id, enrolled, enrolled_at, score, notes)
    //       SELECT ${fe.id}, '${stu}', ${
    //       enrolled ? "true" : "false"
    //     }, ${enrolledAtExpr}, ${scoreExpr}, ${notesExpr}
    //       WHERE NOT EXISTS (SELECT 1 FROM final_exams_students WHERE final_exams_id=${
    //         fe.id
    //       } AND student_id='${stu}');
    //     `);
    //   }
