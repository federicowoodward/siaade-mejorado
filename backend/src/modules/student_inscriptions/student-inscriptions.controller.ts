...
@AllowRoles(ROLE.EXECUTIVE_SECRETARY, ROLE.SECRETARY, ROLE.PRECEPTOR, ROLE.STUDENT)
@Controller('students/inscriptions')
export class StudentInscriptionsController {
  constructor(
    @InjectRepository(FinalExam) private readonly finalRepo: Repository<FinalExam>,
    @InjectRepository(ExamTable) private readonly tableRepo: Repository<ExamTable>,
    @InjectRepository(FinalExamsStudent) private readonly linkRepo: Repository<FinalExamsStudent>,
  ...
  @Get('exam-tables')
  @ApiOperation({ summary: 'Listado de mesas publicadas para el alumno (con validaciones básicas)' })
  @ApiQuery({ name: 'subjectId', required: false, type: Number })
  @ApiQuery({ name: 'from', required: false, type: String })
  @ApiQuery({ name: 'to', required: false, type: String })
  @ApiQuery({ name: 'windowState', required: false, type: String, enum: ['open','upcoming','closed','past','all'] })
  @ApiOkResponse({ description: 'Payload compatible con front StudentInscriptionsService' })
  async listExamTables(
    ...
    for (const r of rows) {
      ...
      if (!groups.has(key)) {
        groups.set(key, {
          ...
          window: { opensAt: r.table_start, closesAt: r.table_end, label: r.table_name, id: Number(r.table_id) },
          subjectOrderNo: r.subject_order_no ? Number(r.subject_order_no) : null,
        });
      }
    ...
    if (studentId) {
      const allFinalIds = result.flatMap((g) => g.availableCalls.map((c: any) => c.id));
      if (allFinalIds.length > 0) {
        const links = await this.linkRepo.find({ where: { finalExamId: In(allFinalIds), studentId } as any });
        const enrolledByFinal = new Set<number>(links.filter((l) => (l as any).enrolledAt).map((l) => l.finalExamId));
        for (const g of result) {
          g.duplicateEnrollment = g.availableCalls.some((c: any) => enrolledByFinal.has(c.id));
        }
      }
    }
    ...
    if (studentId) {
      ...
      if (cs) {
        for (const g of result) {
          if (g.subjectOrderNo) {
            try {
              const validation = await this.prereqSvc.validateEnrollment(cs.careerId, studentId, g.subjectOrderNo);
              if (!validation.canEnroll && Array.isArray(validation.unmet) && validation.unmet.length > 0) {
                ...
                const lines = unmet
                  .map((row) => {
                    const name = row.subject?.subjectName ?? `Materia orden ${row.orderNo}`;
    ...
    const filtered = (windowState && windowState !== 'all')
      ? result.filter((g) => inState(g.window.opensAt, g.window.closesAt) === windowState)
      : result;

    // Shape response as { data: [...] } for front mapper compatibility
    return { data: filtered.map(({ window, subjectOrderNo, ...rest }) => rest) };
  }
